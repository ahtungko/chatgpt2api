                  ...
                  <div key={index} className="space-y-3 border-t border-stone-200 pt-3 first:border-t-0 first:pt-0">
                    ...
                    {type === "tempmail_lol" || type === "cloudflare_temp_email" ? (
                      <div className="space-y-2">
                        ...
                        <Textarea value={domains} onChange={(event) => updateProvider(index, { domain: event.target.value.split(/[\n,]/).map((item) => item.trim()).filter(Boolean) })} placeholder={t("每行一个域名，留空则使用服务默认域名", "One domain per line. Leave blank to use the provider default.")} className="min-h-20 rounded-xl border-stone-200 bg-white font-mono text-xs" disabled={config.enabled} />
                      </div>
                    ) : null}
                  </div>
                );
