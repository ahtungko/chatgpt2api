      ...
      <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
        <DialogHeader className="gap-2">
          ...
          <DialogDescription className="text-sm leading-6">
            {browserPool ? t(`来自 ${browserPool.name || browserPool.base_url}`, `From ${browserPool.name || browserPool.base_url}`) : t("读取到的远程账号列表", "Remote accounts that were loaded")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative min-w-[260px]">
            ...
            <Input
              ...
              placeholder={t("搜索 email 或文件名", "Search email or file name")}
        ...
        <div className="rounded-xl border border-stone-200">
          <div className="flex items-center justify-between border-b border-stone-100 px-4 py-3 text-sm text-stone-500">
            <div className="flex items-center gap-3">
              ...
              <span>{t(`筛选结果 ${filteredFiles.length} 个`, `${filteredFiles.length} filtered results`)}</span>
            </div>
            <span>{t(`已选 ${selectedNames.length} 个`, `${selectedNames.length} selected`)}</span>
          </div>
          <div className="max-h-[420px] overflow-auto">
            {pagedFiles.length === 0 ? (
              <div className="flex items-center justify-center py-12 text-sm text-stone-400">{t("没有匹配的远程账号", "No matching remote accounts")}</div>
            ) : (
        ...
        <div className="flex items-center justify-between text-sm text-stone-500">
          <span>
            {t(
              `第 ${filteredFiles.length === 0 ? 0 : (safeFilePage - 1) * currentPageSize + 1} - ${Math.min(safeFilePage * currentPageSize, filteredFiles.length)} 条，共 ${filteredFiles.length} 条`,
              `${filteredFiles.length === 0 ? 0 : (safeFilePage - 1) * currentPageSize + 1}-${Math.min(safeFilePage * currentPageSize, filteredFiles.length)} of ${filteredFiles.length}`,
            )}
          </span>
          <div className="flex items-center gap-2">
            <Button
