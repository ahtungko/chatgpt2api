      ...
      <CardContent className="space-y-6 p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold tracking-tight">{t("CPA 连接管理", "CPA connections")}</h2>
              <p className="text-sm text-stone-500">{t("先配置连接，再按需查询远程账号并选择导入到本地号池。", "Configure connections first, then browse remote accounts and import the ones you need.")}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {pools.length > 0 ? <Badge className="rounded-md px-2.5 py-1">{t(`${pools.length} 个连接`, `${pools.length} connections`)}</Badge> : null}
        ...
        {isLoadingPools ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-stone-50 px-6 py-10 text-center">
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium text-stone-600">{t("暂无 CPA 连接", "No CPA connections yet")}</p>
              <p className="text-sm text-stone-400">{t("点击「添加连接」保存你的 CLIProxyAPI 信息。", "Click “Add connection” to save your CLIProxyAPI details.")}</p>
            </div>
          </div>
        ) : (
        ...
        <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-500">
          ...
          <ul className="mt-1 list-inside list-disc space-y-0.5">
            <li>{t("页面进入后先读取系统里已配置的 CPA 连接。", "When the page opens, it first loads all configured CPA connections.")}</li>
            <li>{t("点击某个连接的「同步」后，会先读取远程账号列表并展示给前端选择。", "After you click “Sync” on a connection, the remote account list is loaded for you to choose from.")}</li>
            <li>{t("确认选择后，后端后台下载对应 access_token 并导入本地号池。", "After confirmation, the backend downloads the matching access_token values and imports them into the local pool.")}</li>
            <li>{t("前端只轮询导入进度，不直接参与 download。", "The frontend only polls import progress and does not download the tokens itself.")}</li>
          </ul>
        </div>
      </CardContent>
    </Card>
