...
function AccountsPageContent() {
  ...
  const handleExportAccounts = async (format: AccountExportFormat, tokens: string[]) => {
    if (tokens.length === 0) {
      toast.error(t("没有可导出的账户", "No accounts available to export"));
      return;
    }
    ...
    try {
      ...
      toast.success(
        format === "zip"
          ? t("ZIP 压缩包已导出", "ZIP archive exported")
          : t("JSON 文件已导出", "JSON file exported"),
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : t("导出账户失败", "Failed to export accounts");
      toast.error(message);
    } finally {
