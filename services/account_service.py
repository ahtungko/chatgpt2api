...
class AccountService:
    ...
    def _search_account_type(cls, value: object) -> str | None:
        normalized = cls._normalize_account_type(value)
        if normalized in {"Free", "Plus", "ProLite", "Team", "Pro"}:
            return normalized
        if isinstance(value, dict):
            for key in ("type", "plan_type", "account_type", "subscription_plan", "plan"):
                if key in value:
                    found = cls._search_account_type(value.get(key))
                    if found:
                        return found
